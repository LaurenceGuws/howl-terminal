...
| `M8` Host Integration Readiness | `active` | `app_architecture/authorities/M8_FOUNDATION.md` | Architect-owned readiness closure for M8-A/B/C is complete: contract boundary, seam audit (`docs/review/m8/M8_SEAM_AUDIT.md`), and gate matrix are published. Engineer queue is now open for bounded implementation evidence slices; M8-D freeze remains pending. |
