...
| `M5` Runtime Interface | `active` | `app_architecture/authorities/M5_FOUNDATION.md` | M5-A/B complete. M5-A: lifecycle matrix + conformance tests. M5-B: docstrings (all stable methods aligned to contract) + parity matrix (5 mixed host-loop tests proving runtime transparency). M5-C/D pending handoff. |
